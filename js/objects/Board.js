...
class Board extends Phaser.Group {
  ...
  constructor(game, numCols = 8, numRows = 8, numBlockTypes = 7, contentSize = 35, padding = 2, gap = 2) {
    super(game, null);
    ...
    // Make the board initially interactive
    this.isInteractive = true;

    // Create block group
    this.blocks = new Phaser.Group(this.game, null);

    // Create grid and populate it with random values
    this.grid = new Grid(this.numCols, this.numRows);
    this.populateRandomly();

    // Create field group and populate it with fields
    ...
    // Add game objects to this group
  ...
  getBlockSize() {
    return this.contentSize + this.padding * 2;
  }
